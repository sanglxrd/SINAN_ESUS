pacotes <- c("tidyr","gtsummary", "xml2","knitr","kableExtra","DT","tidyverse", "data.table", "readr", "readxl", "writexl", "purrr", "lubridate", "foreign")
...
# Gráfico de Sexo e Faixa Etária: Contagem por sexo e faixa etária com percentuais e números absolutos
...
# Criar o gráfico de pirâmide etária ajustado com barras centralizadas e sem rótulos para "I"
ggplot(sexo_faixa_count, aes(x = Faixa_Etaria, y = Count, fill = CS_SEXO)) +
  ...
  geom_text(aes(label = paste0(abs(Count), " (", round(Percentage, 2), "%)")),
            position = position_stack(vjust = 0.5), size = 3, fontface = "bold", color = "white") +  # Texto centralizado e em negrito
  labs(title = "Pirâmide Etária por Sexo e Faixa Etária", x = "Faixa Etária", y = "Número de Notificações") +
  scale_y_continuous(labels = abs, breaks = seq(-max_count, max_count, by = 5000), limits = c(-max_count, max_count)) +  # Centralizar as barras
